...
import RoundBlueCheckIcon from "components/common/icons/RoundBlueCheckIcon";
...
function ApplicantFormPage() {
  ...
  const [selected, setSelected] = useState(new Array(MOCKUP_EXPORT_TIMES.length).fill(false));
      ...
      <CommonUserResponseFormBox>
              ...
              {currentStep === 1 && (
                <>
                  <Flex
                    ...
                    sx={(theme) => ({
                      padding: "32px",
                      borderRight: showTime ? `1px solid ${theme.colors.gray[1]}` : "none",
                    })}
                  >
                    <Calendar
                      locale="ko"
                      ...
                      excludeDate={(date) =>
                        date.getDay() === 0 || date.getDay() === 6 || date.getDate() === 17
                      }
                      value={value}
        ...
        {currentStep === 2 && (
          <Flex direction="column" sx={{ padding: "32px" }} align="center">
            <RoundBlueCheckIcon style={{ marginBottom: 33, marginTop: 30 }} />
            <Text
              c="gray.8"
              sx={(theme) => ({
                fontSize: 15,
